...
var FilterTable = new Class({
	...
	options: {
		...
		filterType: 'select',
		filterHolder: '.table-filter-holder'
	...
	initialize: function(options)
	{
		...
		tables.each(function(table,index)
		{
			...
			if (table.attributes['class'] && table.hasClass(this.options.filterClass)) 
			{
				...
				filter_holder = table.getElement(this.options.filterHolder);
				...
				switch(this.options.filterType)
				{
					...
					case 'select':
						options = table.getElements('.table-filter-options').get('text').unique().sort();
						var input = new Element('select',{'class': 'filter form-control auto input-sm pull-right',id: 'filter_'+ index});
						options.each(function(value)
						{
							input.adopt(new Element('option', 
							{
								'value': value,
								'text': value
							}));
